...
def create_sum_order_items_df(df):
    sum_order_items_df = df.groupby("product_name")["quantity_sal"].sum().sort_values(ascending=False).reset_index()
...
with st.sidebar:
    ...
    # get start_date and end_date from text_input
...
# Complementing the Dashboard with Various Data Visualizations
...
sns.barplot(x="quantity_sal", y="product_name", data=sum_order_items_df.sort_values(by="quantity_sal", ascending=True).head(5), palette=colors, ax=ax[1])
...
sns.barplot(y="frequency", x="customer_id", data=rfm_df.sort_values("frequency", ascending=False).head(5), palette=colors, ax=ax[1])
ax[1].set_ylabel(None)
...
sns.barplot(y="monetary", x="customer_id", data=rfm_df.sort_values("monetary", ascending=False).head(5), palette=colors, ax=ax[2])
